...
void MainWindow::onLinkHovered(QString link,QString /* title */, QString /*textcontent*/)
{
    if(!link.isEmpty())
    {
        ...
        addressBar->setStyleSheet(QString("QLineEdit { border: 2px solid gray; border-radius: 10px; padding: 0 8px; background: #DDDDFF; selection-background-color: darkgray; }"));
    }
    else
    {
        addressBar->setText(webview->url().toString());
        addressBar->setStyleSheet(QString("QLineEdit { border: 2px solid gray; border-radius: 10px; padding: 0 8px; background: white; selection-background-color: darkgray; }"));
    }
}
